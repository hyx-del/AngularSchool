...
export class GoalComponent implements OnInit {
    ...
    create() {
        ...
        if (params.id) {
            this.http.post("school/teaching/goal/update", params).then(ret => {
                this.notification.success("提示信息", "修改成功");
                this.collection.load();
                this.closeModal();
            });
        } else {
            this.http.post("school/teaching/goal/create", params).then(ret => {
    ...
    remove(data: any) {
        this.modalService.confirm({
            nzTitle: "确定删除？",
            nzOnOk: () => {
                this.http.post("school/teaching/goal/delete", { id: data.id }).then(ret => {
    ...
    changeStatus(data: any) {
        if (data.status == 1) {
            this.http.post("school/teaching/goal/disable", { id: data.id }).then(ret => {
                this.notification.success("提示信息", "禁用成功");
                this.collection.load();
            });
        } else {
            this.http.post("school/teaching/goal/enable", { id: data.id }).then(ret => {
